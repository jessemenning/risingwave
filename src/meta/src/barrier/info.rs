...
impl SharedActorInfos {
    ...
    pub(super) fn recover_database(
        ...
        for (_, _fragment) in database.extract_if(|fragment_id, fragment_infos| {
            ...
            // TODO: Don't send Delete for fragments that belong to idle batch refresh jobs.
            // Idle jobs have no running actors but their fragment mappings must be kept
            // so that queries can still resolve streaming/serving vnode mappings.
            // writer
